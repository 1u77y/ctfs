...
# Allowed hostnames (Docker services + known hosts)
ALLOWED_HOSTNAMES = {
...
def fetch():
    ...
    # Special case: rewrite /openapi.json to /openapi/openapi.json on localhost:9000
    if target == "http://localhost/openapi.json":
        target = "http://localhost:9000/openapi/openapi.json"

    parsed = urlparse(target)
    if parsed.scheme not in ("http", "https") or not parsed.hostname:
        abort(400, "unsupported URL")

    hostname = parsed.hostname
    port = parsed.port or (80 if parsed.scheme == "http" else 443)

    # Rewrite localhost:9000 -> admin_api:9000 inside Docker
    ...
    try:
        ...
        resp = requests.get(target, headers=headers, timeout=REQUEST_TIMEOUT, allow_redirects=True)
    ...
    # HTML rewriting for Swagger UI pages
    if content_type and "text/html" in content_type.lower():
        try:
            text = content.decode("utf-8", errors="ignore")

            # Inject <base> tag to make relative paths resolve correctly
            base_tag = f'<base href="/fetch?url={quote(target, safe="")}" />'
            text = re.sub(r'(?i)<head\b[^>]*>', lambda m: m.group(0) + base_tag, text, count=1)

            # Rewrite relative Swagger static assets (JS/CSS)
            def rewrite_static(m):
                url = m.group("url").lstrip('/')
                if url.startswith("swagger"):
                    full_url = f"http://localhost:9000/openapi/{url}"
                else:
            ...
            (function() {{
              ...
              // Run after DOM loads and periodically in case UI loads late
...
#     # HTML rewriting for relative paths (Swagger UI fix)
#     if content_type and "text/html" in content_type.lower():
#         try:
#             text = content.decode("utf-8", errors="ignore")

#             # Inject <base> with URL-encoded target
#             base_tag = f'<base href="/fetch?url={quote(target, safe="")}" />'
#             text = re.sub(r'(?i)<head\b[^>]*>', lambda m: m.group(0) + base_tag, text, count=1)

#             # Rewrite static / swagger URLs to go through /fetch
#             def rewrite_static(m):
#                 url = m.group("url").lstrip('/')
#                 if url.startswith("swagger"):
#                     full_url = f"http://localhost:9000/openapi/{url}"
...
#             # Replace Python None → JS null in <script> blocks
#             def fix_none_in_js(html: str) -> str:
#                 def repl(m):
#                     return m.group(0).replace("None", "null")
#                 return re.sub(r"<script.*?>.*?</script>", repl, text, flags=re.DOTALL|re.IGNORECASE)
#             text = fix_none_in_js(text)

#             # Override Swagger download URL input
#             override_script = f"""
#             <script>
#             setTimeout(function(){{
#                 var input = document.querySelector(".download-url-input");
#                 if(input) {{
#                     input.value = "{quote('http://localhost:9000/openapi/openapi.json', safe='/:')}";
#                 }}
#                 // Force Swagger UI to load the spec from the proxied URL
#                 if(window.ui && ui.specActions){{
#                     ui.specActions.updateSpec("{quote('http://localhost:9000/openapi/openapi.json', safe='/:')}");
#                 }}
#             }}, 100);
#             </script>
#             """
